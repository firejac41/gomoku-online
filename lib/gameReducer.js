      ...
      if (ability === "raid") {
        ...
        for (let yy = 0; yy < BOARD_SIZE; yy++) {
          for (let xx = 0; xx < BOARD_SIZE; xx++) {
            if (state.board[yy][xx] !== opponentColorForRaid) continue;
            // 예방으로 지정된 돌은 무작위 대상 풀에서 아예 제외됨 (습격처럼 무작위 선택인 공격은 그 돌을 뽑지 못함)
            if (preventedCell && preventedCell.x === xx && preventedCell.y === yy) continue;
            opponentStones.push({ x: xx, y: yy });
          }
        }
        ...
        let stateAfterSurvivor = survivorPatch ? { ...state, ...survivorPatch } : state;
        const interestPatch = targets.length > 0 ? triggerInterestIfNeeded(stateAfterSurvivor, opponent) : null;
        if (interestPatch) stateAfterSurvivor = { ...stateAfterSurvivor, ...interestPatch };
        return {
          ...stateAfterSurvivor,
          board: newBoard,
          ...
          forbiddenMessage:
            ...
            "'습격'으로 상대 돌 " + targets.length + "개를 제거했어요!",
          forbiddenToken: state.forbiddenToken + 1,
        };
      }
