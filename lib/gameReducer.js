...
export function initialGameState(isOnlineMode = false) {
  const base = {
    ...
    recruitCooldown: { 1: 0, 2: 0 }, // 영입 재사용 대기시간 - 사용하면 6으로 리셋
    ...
    // 두 번째 개편(골드/프리즘 재설계) 이후 신규 카드들
    vinegarCooldown: { 1: 0, 2: 0 }, // 초치기 재사용 대기시간 - 사용하면 5로 리셋
    rerollJamPending: { 1: false, 2: false }, // 초치기에 당한 피해자 - 이 플레이어의 다음 증강 선택에서 슬롯 1개가 미리 리롤된 것으로 처리됨
    fogZoneCooldown: { 1: 0, 2: 0 }, // 안개 재사용 대기시간 - 사용하면 6으로 리셋
    fogZoneCells: { 1: [], 2: [] }, // 이 플레이어(피해자) 화면에서만 안개로 가려진 칸: [{x,y,turnsLeft}]
    evadeCooldown: { 1: 0, 2: 0 }, // 회피 재사용 대기시간 - 사용하면 5로 리셋
    evadeCell: { 1: null, 2: null }, // 회피로 보호해둔 내 돌 좌표 {x,y} - 강탈 1회를 막아주고 소모됨
    tierPenalty: { 1: 0, 2: 0 }, // 등급 강탈에 당한 피해자 - 다음 증강 선택 등급을 이만큼 단계 하락시킴(소비되면 0으로 리셋)
...
const INSTANT_ACTIVATE_MESSAGE = {
  ...
  tierShift: "'등급 강탈'로 내 다음 증강 선택 등급은 오르고 상대는 내려갑니다!",
  absoluteZero: "'절대영도'가 발동돼서 이제부터 상대는 액티브 능력을 쓸 수 없습니다!",
};
function activateInstantAugments(state, grantedAugments, player) {
  ...
  for (const augment of grantedAugments) {
    ...
    // 등급 강탈: 내 다음 증강 선택 등급 +1단계(이자와 같은 interestBonusTier 재사용), 상대 다음 증강 선택 등급 -1단계(tierPenalty)
    if (augment.id === "tierShift") {
      const opp = player === 1 ? 2 : 1;
      const baseInterest = patch.interestBonusTier || state.interestBonusTier;
      patch.interestBonusTier = { ...baseInterest, [player]: (baseInterest[player] || 0) + 1 };
      const basePenalty = patch.tierPenalty || state.tierPenalty;
      patch.tierPenalty = { ...basePenalty, [opp]: (basePenalty[opp] || 0) + 1 };
      messages.push(INSTANT_ACTIVATE_MESSAGE.tierShift);
    }
    // 절대영도: 별도 상태 없이 소유 여부만으로 판정(USE_ABILITY 진입부에서 체크) - 여기서는 발동 안내 메시지만 띄움
    if (augment.id === "absoluteZero") {
      messages.push(INSTANT_ACTIVATE_MESSAGE.absoluteZero);
    }
  }
...
// 등급 강탈에 당했을 때 등급을 한 단계씩 하락시킴 (프리즘→골드→실버, 실버는 이미 최저 등급이라 그대로 유지)
function downgradeTier(tier) {
  if (tier === "prism") return "gold";
  return "silver";
}

// 규칙 완화(diagBoost)처럼 다른 카드보다 파워가 세다고 판단해 등장 확률을 추가로 낮추고 싶은 카드용 -
// 뽑힌 카드가 이 목록에 있으면 지정된 확률로 같은 등급 안에서 재추첨함(재추첨 결과가 없으면 원래 카드 유지)
const RARITY_DAMPED_IDS = { diagBoost: 0.5 };
function dampRarity(card, tier, hardExclude, seenIds, myColor) {
  if (!card) return card;
  const dampChance = RARITY_DAMPED_IDS[card.id];
  if (!dampChance || Math.random() >= dampChance) return card;
  return drawOneOfTier(tier, [...hardExclude, card.id], seenIds, myColor) || card;
}
...
function buildAugmentChoices(state, player, roundIndex) {
  ...
  // 등급 강탈에 당했으면(누적) 승급을 반영한 뒤에 등급을 하락시킴 - 소비(리셋)도 CLICK_CELL 쪽에서 처리
  const penaltyStacks = state.tierPenalty[player] || 0;
  for (let i = 0; i < penaltyStacks; i++) effectiveTier = downgradeTier(effectiveTier);
  ...
  for (let i = 0; i < remaining; i++) {
    ...
    const drawExclude = [...hardExclude, ...usedSoFarIds];
    const card = dampRarity(drawOneOfTier(effectiveTier, drawExclude, seenIds, myColor), effectiveTier, drawExclude, seenIds, myColor);
...
// "강탈"도 같은 부류 - 뽑는 즉시 대상을 지정해야만 효과가 생기고, 유효한 대상이 하나도 없으면(0수 시작 증강 등)
// 재시도할 방법 없이 영원히 죽는 카드라 여기 포함시킴
const IMMEDIATE_TARGET_EXCLUDE_IDS = ["banZone", "permaBlock", "gamble", "steal"];
...
function buildStartAugmentChoices(state, player) {
  ...
  const drawn = [];
  for (let i = 0; i < 3; i++) {
    const drawExclude = [...START_DRAFT_EXCLUDE_IDS, ...modeExclude, ...drawn.map((c) => c.id)];
    const card = dampRarity(drawOneOfTier(roundTier, drawExclude, seenIds, myColor), roundTier, drawExclude, seenIds, myColor);
    if (card) drawn.push(card);
  }
  return {
  ...
  switch (action.type) {
    case "CLICK_CELL": {
      ...
      if (currentColor === 1) {
        ...
        // 금수 면제: 흑돌 전용 카드, 보유하는 동안 무조건 렌주룰 금수가 면제됨
        const isForbiddenFreeExempt = ownedIds.includes("forbiddenFree");
        if (!isBalanceExempt && !isForbiddenFreeExempt) {
      ...
      if (!opponentOwnedIds.includes("fortress") && (hasCapture || hasOthello)) {
        for (const c of findCaptures(newBoard, x, y, currentColor)) {
          ...
          } else if (hasOthello && Math.random() < 0.5) {
            // 오델로: 낀 돌마다 50% 확률로만 실제 발동 (발동 자체가 확률적이라는 설계 확정 - 등장 확률이 아니라 효과 자체를 완화)
      ...
      const decayedRecruitCooldown = isRushBonusStone
        ? state.recruitCooldown[currentPlayer]
        : Math.max(0, state.recruitCooldown[currentPlayer] - 1);
      ...
      const decayedVinegarCooldown = isRushBonusStone
        ? state.vinegarCooldown[currentPlayer]
        : Math.max(0, state.vinegarCooldown[currentPlayer] - 1);
      const decayedFogZoneCooldown = isRushBonusStone
        ? state.fogZoneCooldown[currentPlayer]
        : Math.max(0, state.fogZoneCooldown[currentPlayer] - 1);
      const decayedEvadeCooldown = isRushBonusStone
        ? state.evadeCooldown[currentPlayer]
        : Math.max(0, state.evadeCooldown[currentPlayer] - 1);
      ...
      // 안개(새 버전): 나에게 걸린 안개 구역도 내가 한 수 둘 때마다 남은 턴이 1씩 줄고, 0이 되면 사라짐
      const decayedFogZoneCells = isRushBonusStone
        ? state.fogZoneCells[currentPlayer]
        : state.fogZoneCells[currentPlayer].map((c) => ({ ...c, turnsLeft: c.turnsLeft - 1 })).filter((c) => c.turnsLeft > 0);
      ...
      const baseState = {
        ...
        recruitCooldown: { ...state.recruitCooldown, [currentPlayer]: decayedRecruitCooldown },
        ...
        vinegarCooldown: { ...state.vinegarCooldown, [currentPlayer]: decayedVinegarCooldown },
        fogZoneCooldown: { ...state.fogZoneCooldown, [currentPlayer]: decayedFogZoneCooldown },
        evadeCooldown: { ...state.evadeCooldown, [currentPlayer]: decayedEvadeCooldown },
        fogZoneCells: { ...state.fogZoneCells, [currentPlayer]: decayedFogZoneCells },
      ...
      if (!isRushBonusStone && newStonesPlaced[currentPlayer] % 4 === 0) {
        ...
        // 증강 풀이 한정돼 있어서, 이미 전부 다 보유하면 이번 회차에 새로 보여줄 카드가 하나도 없을 수 있음
        ...
        const bonusRerolls = stockpileCard
          ...
          : 0;
        // 초치기에 당했으면(rerollJamPending) 이번 선택지 슬롯 하나를 미리 리롤된 것으로 처리해서 실제 리롤 기회를 1회 줄임
        const rerolledSlotsInit = choices.map(() => false);
        if (state.rerollJamPending[currentPlayer]) {
          rerolledSlotsInit[Math.floor(Math.random() * rerolledSlotsInit.length)] = true;
        }
        return {
          ...baseState,
          usedAugmentIds: { ...state.usedAugmentIds, [currentPlayer]: usedAugmentIds },
          ...
          tierPenalty: { ...state.tierPenalty, [currentPlayer]: 0 },
          rerollJamPending: { ...state.rerollJamPending, [currentPlayer]: false },
          differentiatedDraftPending: { ...state.differentiatedDraftPending, [currentPlayer]: false },
          augmentSelect: {
            player: currentPlayer,
            choices,
            rerolledSlots: rerolledSlotsInit,
            differentiated,
    ...
    case "PICK_AUGMENT": {
      ...
      // 강탈: 고르자마자 즉시 대상을 지정해야 함 - 유효한 대상(고립되지 않고 3목 이상 라인에 안 낀 상대 돌)이
      // 하나도 없으면(예: 극초반) pendingTarget을 만들지 않고 카드 소모 없이 안내만 띄우고 정상 진행
      if (augment.id === "steal") {
        const stealOppColor = colorForPlayer(otherPlayer(player), state.roleSwapActive);
        const hasValidStealTarget = findStealTargets(state.board, stealOppColor).length > 0;
        if (!hasValidStealTarget) {
          return {
            ...state,
            ownedAugments: newOwned,
            augmentSelect: null,
            currentPlayer: keepTurn ? player : otherPlayer(player),
            forbiddenMessage: "'강탈'을 뽑았지만 지금은 데려올 수 있는 상대 돌이 없어요",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        return { ...state, ownedAugments: newOwned, augmentSelect: null, pendingTarget: { player, kind: "steal", need: 1, selected: [], keepTurn } };
      }
    ...
    case "REROLL_SLOT": {
      ...
      const rerollExclude = [...otherShownIds, ...ownedIds, ...opponentOwnedIds, ...stageExcludeIds, ...getModeExcludeIds(state), currentCard.id];
      const rerollColor = colorForPlayer(player, state.roleSwapActive);
      const newCard = dampRarity(
        drawOneOfTier(currentCard.tier, rerollExclude, seenIds, rerollColor),
        currentCard.tier,
        rerollExclude,
        seenIds,
        rerollColor
    ...
    case "USE_ABILITY": {
      ...
      // 절대영도: 상대가 이 카드를 갖고 있으면 내 모든 액티브 능력이 봉인됨 (프리즘이라 getActiveAugmentIds로
      // 교도소 필터까지 정확히 반영)
      if (getActiveAugmentIds(state, opponent).includes("absoluteZero")) {
        return {
          ...state,
          forbiddenMessage: "상대의 '절대영도'에 막혀서 능력을 쓸 수 없어요",
          forbiddenToken: state.forbiddenToken + 1,
        };
      }

      if (ability === "vinegar") {
        if (state.vinegarCooldown[player] > 0) {
          return {
            ...state,
            forbiddenMessage: "'초치기'는 아직 재사용 대기 중이에요 (" + state.vinegarCooldown[player] + "수 남음)",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        return {
          ...state,
          rerollJamPending: { ...state.rerollJamPending, [opponent]: true },
          vinegarCooldown: { ...state.vinegarCooldown, [player]: 5 },
          forbiddenMessage: "'초치기'로 상대의 다음 증강 선택 리롤 1회를 줄였어요!",
          forbiddenToken: state.forbiddenToken + 1,
        };
      }

      if (ability === "fogZone") {
        if (state.fogZoneCooldown[player] > 0) {
          return {
            ...state,
            forbiddenMessage: "'안개'는 아직 재사용 대기 중이에요 (" + state.fogZoneCooldown[player] + "수 남음)",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        return { ...state, pendingTarget: { player, kind: "fogZone", need: 1, selected: [] } };
      }

      if (ability === "evade") {
        if (state.evadeCooldown[player] > 0) {
          return {
            ...state,
            forbiddenMessage: "'회피'는 아직 재사용 대기 중이에요 (" + state.evadeCooldown[player] + "수 남음)",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        const evadeColor = colorForPlayer(player, state.roleSwapActive);
        if (countStones(state.board, evadeColor) === 0) {
          return {
            ...state,
            forbiddenMessage: "회피: 보호할 내 돌이 없어요",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        return { ...state, pendingTarget: { player, kind: "evade", need: 1, selected: [] } };
      }
      ...
      if (ability === "recruit") {
        if (state.recruitCooldown[player] > 0) {
          return {
            ...state,
            forbiddenMessage: "'영입'은 아직 재사용 대기 중이에요 (" + state.recruitCooldown[player] + "수 남음)",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        const recruitMyColor = colorForPlayer(player, state.roleSwapActive);
        const recruitOpponentColor = colorForPlayer(opponent, state.roleSwapActive);
        const recruitTargets = findIsolatedStones(state.board, recruitOpponentColor);
        if (recruitTargets.length === 0) {
          return {
            ...state,
            forbiddenMessage: "영입: 데려올 수 있는 상대 외톨이 돌이 없어요",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        const recruitTarget = pickRandom(recruitTargets, 1)[0];
        const recruitBoard = state.board.map((row) => row.slice());
        // 자리는 그대로 두고 색만 내 돌로 바꿈 - 외톨이 돌은 색을 바꿔도 여전히 외톨이라 판세 불변
        recruitBoard[recruitTarget.y][recruitTarget.x] = recruitMyColor;
        // 색을 뺏긴 돌이 상대의 마지막 수였다면 마지막 수 표시가 더 이상 상대 돌을 가리키지 않게 지움
        const recruitOppLast = state.lastMove[opponent];
        const recruitWasLast = recruitOppLast && recruitOppLast.x === recruitTarget.x && recruitOppLast.y === recruitTarget.y;
        return {
          ...state,
          board: recruitBoard,
          lastMove: recruitWasLast ? { ...state.lastMove, [opponent]: null } : state.lastMove,
          recruitCooldown: { ...state.recruitCooldown, [player]: 6 },
          currentPlayer: opponent, // 내 턴을 통째로 소모 - 도토리 던지기와 같은 급의 순수 심술
          forbiddenMessage: "'영입'으로 상대 외톨이 돌 하나를 내 돌로 데려왔어요! (턴 소모)",
          forbiddenToken: state.forbiddenToken + 1,
        };
      }
    ...
    case "TARGET_CELL": {
      ...
      if (kind === "fogZone") {
        ...
        for (let dy = -1; dy <= 1; dy++) {
          for (let dx = -1; dx <= 1; dx++) {
            ...
            region.push({ x: nx, y: ny, turnsLeft: 3 });
          }
        }
        return {
          ...state,
          pendingTarget: null,
          fogZoneCooldown: { ...state.fogZoneCooldown, [player]: 6 },
          fogZoneCells: { ...state.fogZoneCells, [opponent]: [...state.fogZoneCells[opponent], ...region] },
          forbiddenMessage: "'안개'로 상대 화면 일부가 가려졌어요!",
          forbiddenToken: state.forbiddenToken + 1,
        };
      }
      ...
      if (kind === "evade") {
        ...
        return {
          ...state,
          pendingTarget: null,
          evadeCell: { ...state.evadeCell, [player]: { x, y } },
          evadeCooldown: { ...state.evadeCooldown, [player]: 5 },
          forbiddenMessage: "'회피'로 내 돌 하나를 보호했어요!",
          forbiddenToken: state.forbiddenToken + 1,
        };
      }
      ...
      if (kind === "steal") {
        ...
        if (!hasNeighborStone(state.board, x, y)) return state;
        if (getRunCounts(state.board, x, y, opponentColor).some((c) => c >= 3)) return state;

        // 회피: 피해자가 미리 이 정확한 칸을 지정해뒀으면 강탈이 막힘 (소모성, 1회 보호)
        const evadeMark = state.evadeCell[opponent];
        if (evadeMark && evadeMark.x === x && evadeMark.y === y) {
          return {
            ...state,
            pendingTarget: null,
            evadeCell: { ...state.evadeCell, [opponent]: null },
            currentPlayer: state.pendingTarget.keepTurn ? player : opponent,
            forbiddenMessage: "상대가 '회피'로 그 돌을 지켰어요!",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }

        const stealBoard = state.board.map((row) => row.slice());
        stealBoard[y][x] = playerColor;
        let stealLastMove = state.lastMove;
        const stealOppLast = state.lastMove[opponent];
        if (stealOppLast && stealOppLast.x === x && stealOppLast.y === y) {
          stealLastMove = { ...stealLastMove, [opponent]: null };
        }

        // 역습: 피해자가 역습을 갖고 있으면 즉시 공격자의 돌 하나(비고립, 3목 미만)를 다시 뺏어감
        let retaliateMessage = "";
        if (getActiveAugmentIds(state, opponent).includes("retaliate")) {
          const retaliateTargets = findStealTargets(stealBoard, playerColor);
          if (retaliateTargets.length > 0) {
            const rt = pickRandom(retaliateTargets, 1)[0];
            stealBoard[rt.y][rt.x] = opponentColor;
            const myLast = stealLastMove[player];
            if (myLast && myLast.x === rt.x && myLast.y === rt.y) {
              stealLastMove = { ...stealLastMove, [player]: null };
            }
            retaliateMessage = " 상대가 '역습'으로 내 돌 하나를 다시 뺏어갔어요!";
          }
        }
        ...
        return {
          ...state,
          board: stealBoard,
          lastMove: stealLastMove,
          pendingTarget: null,
          currentPlayer: state.pendingTarget.keepTurn ? player : opponent,
          forbiddenMessage: "'강탈'로 상대 돌 하나를 데려왔어요!" + retaliateMessage,
          forbiddenToken: state.forbiddenToken + 1,
        };
      }
