...
export function initialGameState(isOnlineMode = false) {
  const base = {
    ...
    // 증강 전면 개편(2026-07-18) 이후 신규 카드 3종 - 전부 게임 상태(보드/턴/승패)에 절대 영향 없는 연출용 액티브
    jeerCooldown: { 1: 0, 2: 0 }, // 약올리기 재사용 대기시간 - 사용하면 4로 리셋
    startleCooldown: { 1: 0, 2: 0 }, // 놀래키기 재사용 대기시간 - 사용하면 4로 리셋
    startleToken: 0, // 놀래키기를 쓸 때마다 1씩 증가 - UI가 이 값 변화를 감지해서 판 흔들림 연출을 재생
    firecrackerCooldown: { 1: 0, 2: 0 }, // 폭죽 세례 재사용 대기시간 - 사용하면 4로 리셋
    firecrackerToken: 0, // 폭죽 세례를 쓸 때마다 1씩 증가 - UI가 이 값 변화를 감지해서 폭죽 연출을 재생
...
export function gameReducer(state, action) {
  switch (action.type) {
    case "CLICK_CELL": {
      ...
      // 약올리기/놀래키기/폭죽 세례도 같은 재사용 대기시간 방식 (보너스 돌은 카운트 안 함)
      const decayedJeerCooldown = isRushBonusStone
        ? state.jeerCooldown[currentPlayer]
        : Math.max(0, state.jeerCooldown[currentPlayer] - 1);
      const decayedStartleCooldown = isRushBonusStone
        ? state.startleCooldown[currentPlayer]
        : Math.max(0, state.startleCooldown[currentPlayer] - 1);
      const decayedFirecrackerCooldown = isRushBonusStone
        ? state.firecrackerCooldown[currentPlayer]
        : Math.max(0, state.firecrackerCooldown[currentPlayer] - 1);
      ...
      const baseState = {
        ...
        jeerCooldown: { ...state.jeerCooldown, [currentPlayer]: decayedJeerCooldown },
        startleCooldown: { ...state.startleCooldown, [currentPlayer]: decayedStartleCooldown },
        firecrackerCooldown: { ...state.firecrackerCooldown, [currentPlayer]: decayedFirecrackerCooldown },
    ...
    case "USE_ABILITY": {
      ...
      // 약올리기/놀래키기/폭죽 세례: 셋 다 board/턴/승패 등 실제 게임 상태를 절대 건드리지 않는 순수 연출용 액티브 -
      // 대비태세와 같은 패턴(쿨다운만 있고 턴은 안 넘어감)으로 아무 때나 눌러도 안전함
      if (ability === "jeer") {
        if (state.jeerCooldown[player] > 0) {
          return {
            ...state,
            forbiddenMessage: "'약올리기'는 아직 재사용 대기 중이에요 (" + state.jeerCooldown[player] + "수 남음)",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        const line = pickRandom(JEER_LINES, 1)[0];
        return {
          ...state,
          jeerCooldown: { ...state.jeerCooldown, [player]: 4 },
          forbiddenMessage: "'약올리기': " + line,
          forbiddenToken: state.forbiddenToken + 1,
        };
      }

      if (ability === "startle") {
        if (state.startleCooldown[player] > 0) {
          return {
            ...state,
            forbiddenMessage: "'놀래키기'는 아직 재사용 대기 중이에요 (" + state.startleCooldown[player] + "수 남음)",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        return {
          ...state,
          startleCooldown: { ...state.startleCooldown, [player]: 4 },
          startleToken: state.startleToken + 1,
          forbiddenMessage: "'놀래키기'로 판을 흔들었어요!",
          forbiddenToken: state.forbiddenToken + 1,
        };
      }

      if (ability === "firecracker") {
        if (state.firecrackerCooldown[player] > 0) {
          return {
            ...state,
            forbiddenMessage: "'폭죽 세례'는 아직 재사용 대기 중이에요 (" + state.firecrackerCooldown[player] + "수 남음)",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        return {
          ...state,
          firecrackerCooldown: { ...state.firecrackerCooldown, [player]: 4 },
          firecrackerToken: state.firecrackerToken + 1,
          forbiddenMessage: "'폭죽 세례'를 터트렸어요!",
          forbiddenToken: state.forbiddenToken + 1,
        };
      }
