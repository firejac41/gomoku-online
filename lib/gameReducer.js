...
// 교도소가 발동 중이면 프리즘 등급 증강체는 전부 없는 셈 치고 판정 (자기 자신 포함, 양쪽 다 적용)
  ...
  switch (action.type) {
    case "CLICK_CELL": {
      ...
      if (isBlocked) {
        return {
          ...state,
          forbiddenMessage: "여기는 막혀 있어서 놓을 수 없어요",
          forbiddenToken: state.forbiddenToken + 1,
        };
      }
      ...
      // 렌주룰 금수 판정 (흑돌만 적용, 렌주룰 자체는 증강체와 무관 / 균형 보유 시 열세면 면제)
      if (currentPlayer === 1) {
      ...
      if (newStonesPlaced[currentPlayer] % 4 === 0) {
        ...
        // 축적: 내가 가진 증강체 1개당 이번 증강 선택에서 보너스 리롤 1회씩
    ...
    case "PICK_AUGMENT": {
      ...
      if (augment.id === "prison") {
        patch.prisonActive = true;
        patch.forbiddenMessage = "'교도소'가 발동돼서 이제부터 양쪽 모두 프리즘 효과가 사라집니다!";
        patch.forbiddenToken = state.forbiddenToken + 1;
      }
    ...
    case "USE_ABILITY": {
      ...
      if (ability === "raid") {
        ...
        for (let yy = 0; yy < BOARD_SIZE; yy++) {
          for (let xx = 0; xx < BOARD_SIZE; xx++) {
            if (state.board[yy][xx] === opponent) opponentStones.push({ x: xx, y: yy });
          }
        }
      ...
      if (ability === "boardFlip") {
        ...
        for (const s of spots) newBoard[s.y][s.x] = player;
        return {
          ...state,
          board: newBoard,
      ...
      if (ability === "undo") {
        ...
        if (victimOwnedIds.includes("counterStrike") && !state.oneTimeUsed[opponent]?.counterStrike) {
          ...
          if (attackerLast && newBoard[attackerLast.y][attackerLast.x] === player) {
    ...
    case "TARGET_CELL": {
      ...
      if (kind === "relocate") {
        if (!state.pendingTarget.sourceCell) {
          if (state.board[y][x] !== player) return state;
      ...
      if (kind === "collapse") {
        ...
        for (let dy = -1; dy <= 1; dy++) {
          for (let dx = -1; dx <= 1; dx++) {
            const cx = x + dx;
            const cy = y + dy;
            if (cx >= 0 && cx < BOARD_SIZE && cy >= 0 && cy < BOARD_SIZE) newBoard[cy][cx] = 0;
          }
        }
        return {
          ...state,
          board: newBoard,
          pendingTarget: null,
          oneTimeUsed: markUsed(state, player, "collapse"),
          forbiddenMessage: "'붕괴'로 3x3 구역이 사라졌어요!",
          forbiddenToken: state.forbiddenToken + 1,
          currentPlayer: opponent,
        };
      }
