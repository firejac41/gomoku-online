...
function pickRandomLegalCell(state, player) {
  ...
  const ringBounds = getRingBounds(state.ringStartMove, totalStonesPlacedForRing, state.ringTarget);
...
export function gameReducer(state, action) {
  switch (action.type) {
    case "CLICK_CELL": {
      ...
      // 링 위에서 싸우자: 이번 수로 총 착수 수가 늘어나 링이 새로 한 겹 더 좁혀졌으면, 새로 경계 밖이 된 칸의 돌을 삭제
      // - 상대 공격이 아니라 환경 요인(좁아지는 전장)이라 생존자/여진/철옹성 등 "제거 방어" 계열과 무관하게 무조건 삭제됨
      const totalStonesPlacedAfterRing = newStonesPlaced[1] + newStonesPlaced[2];
      const newRingBounds = getRingBounds(state.ringStartMove, totalStonesPlacedAfterRing, state.ringTarget);
