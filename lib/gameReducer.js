...
function buildAugmentChoices(state, player, roundIndex) {
  ...
  // 역전의 근성: 이 회차 시점에 내 증강 개수가 상대보다 적으면 이번 회차만 한 단계 승급 (누적 안 됨, 격차가 좁혀지면 다음 회차부터 자동으로 꺼짐)
  if (ownedIds.includes("underdogGrit") && opponentOwnedIds.length > ownedIds.length) {
