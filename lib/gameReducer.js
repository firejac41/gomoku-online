...
export function initialGameState(isOnlineMode = false) {
  const base = {
    ...
    dobalTurnsLeft: { 1: 0, 2: 0 }, // 도발 - 이 플레이어(피해자)의 제한시간이 절반으로 줄어드는 남은 자기 턴 수
...
export function gameReducer(state, action) {
  switch (action.type) {
    case "CLICK_CELL": {
      ...
      // 도발: 피해자 본인이 한 수 둘 때마다 1턴씩 소진 (보너스 돌은 카운트 안 함)
      const decayedDobalTurnsLeft = isRushBonusStone
        ? state.dobalTurnsLeft[currentPlayer]
        : Math.max(0, state.dobalTurnsLeft[currentPlayer] - 1);
      ...
      const baseState = {
        ...
        dobalTurnsLeft: { ...state.dobalTurnsLeft, [currentPlayer]: decayedDobalTurnsLeft },
    ...
    case "USE_ABILITY": {
      ...
      if (ability === "dobal") {
        const dobalImmunity = checkImmunity(state, opponent);
        if (dobalImmunity.immune) {
          return {
            ...state,
            ...consumePrepStance(state, opponent, dobalImmunity.reason),
            oneTimeUsed: applyImmunityConsumption(markUsed(state, player, "dobal"), opponent, dobalImmunity.reason),
            forbiddenMessage: immunityMessage(dobalImmunity.reason),
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        return {
          ...state,
          dobalTurnsLeft: { ...state.dobalTurnsLeft, [opponent]: 3 },
          oneTimeUsed: markUsed(state, player, "dobal"),
          forbiddenMessage: "'도발'로 상대 제한시간을 3턴 동안 절반으로 줄였어요!",
          forbiddenToken: state.forbiddenToken + 1,
        };
      }
    ...
    case "TARGET_CELL": {
      ...
      if (kind === "removeStone") {
        ...
        return {
          ...
          forbiddenToken:
            immunity.immune || extraMessage || counterMessage || survivorPatch || interestPatch || backlashMessage
      ...
      if (kind === "plague") {
        ...
        return {
          ...
          forbiddenMessage: immunity.immune
            ? immunityMessage(immunity.reason)
            : (survivorPatch ? survivorPatch.forbiddenMessage + " " : "") + (interestPatch ? interestPatch.forbiddenMessage : ""),
          forbiddenToken: immunity.immune || survivorPatch || interestPatch ? state.forbiddenToken + 1 : state.forbiddenToken,
          currentPlayer: immunity.immune ? player : opponent,
        };
      }
