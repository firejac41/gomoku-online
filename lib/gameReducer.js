...
export function initialGameState() {
  return {
    ...
    forbiddenMessage: "",
    forbiddenToken: 0,
    augmentSelect: null, // { player, choices: [augment,...], rerolledSlots: [bool,...], differentiated, isGamble? }
    ...
    differentiatedDraftPending: { 1: false, 2: false }, // 저울질 - 다음 증강 선택에서 상대가 가진 증강체 제외
...
// 4턴 달성 시 새 증강 선택 카드 뽑기 (먼저 보기/더블 초이스/인색/저울질 반영, 등급 조합은 draftTierPlan에서 공유)
function buildAugmentChoices(state, player, roundIndex) {
  ...
  // 인색(카드 1장 줄임)/더블 초이스(1장 늘림)는 그 회차 등급 조합 위에 슬롯 개수만 조정
  let slotTiers = combo;
  if (state.stingyDraft[player]) slotTiers = slotTiers.slice(0, Math.max(1, slotTiers.length - 1));
  if (state.doubleChoicePending[player]) slotTiers = [...slotTiers, pickRandom(["silver", "gold"], 1)[0]];

  const hardExclude = [...ownedIds, ...stageExcludeIds, ...extraExclude];
  const drawn = [];
  let remainingTiers = slotTiers;

  if (peeked) {
    drawn.push(peeked);
    remainingTiers = slotTiers.slice(1); // 먼저 보기로 예약된 카드가 한 자리를 차지
  }
...
export function gameReducer(state, action) {
  switch (action.type) {
    case "CLICK_CELL": {
      ...
      if (newStonesPlaced[currentPlayer] % 4 === 0) {
        ...
        return {
          ...
          augmentSelect: { player: currentPlayer, choices, rerolledSlots: choices.map(() => false), differentiated },
    ...
    // 즉시 타겟팅이 필요 없는 증강체를 원할 때 사용 (양수겹침/직전 무르기/되돌리기/돌 제거/동전 던지기/속박/인색/거래/저울질/무위전변)
    case "USE_ABILITY": {
    ...
    case "TARGET_CELL": {
      ...
      if (kind === "watchtower") {
        ...
        return {
          ...
          oneTimeUsed: applyImmunityConsumption(markUsed(state, player, "watchtower"), opponent, immunity.reason),
          forbiddenMessage: immunity.immune ? immunityMessage(immunity.reason) : "",
          forbiddenToken: immunity.immune ? state.forbiddenToken + 1 : state.forbiddenToken,
          currentPlayer: immunity.immune ? player : opponent,
        };
