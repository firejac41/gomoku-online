...
function triggerAmbushIfNeeded(state, victim) {
  ...
  const [bonus] = drawFromPool(victimOwnedIds, seenIds, 1, colorForPlayer(victim, state.roleSwapActive));
...
function triggerSurvivorQuestIfNeeded(state, victim) {
  ...
  const seenIds = state.usedAugmentIds[victim];
  const bonus = drawSeveralOfTier("silver", 2, victimOwnedIds, seenIds, colorForPlayer(victim, state.roleSwapActive));
...
function buildAugmentChoices(state, player, roundIndex) {
  ...
  const effectiveTier = state.conquerorPending[player]
    ? "prism"
    : roundTier === "silver" && ownedIds.includes("lateBloomer") && totalStonesPlaced >= 16
      ? "gold"
      : roundTier;
...
function buildStartAugmentChoices(state, player) {
  ...
  const drawn = [];
  for (let i = 0; i < 3; i++) {
    const card = drawOneOfTier(roundTier, [...START_DRAFT_EXCLUDE_IDS, ...drawn.map((c) => c.id)], seenIds, myColor);
    if (card) drawn.push(card);
  }
  return {
  ...
  switch (action.type) {
    case "CLICK_CELL": {
      ...
      if (currentColor === 1) {
        ...
        // 역감시: 흑돌이 총 8수 이내에 처음 만드는 열린 3목을 감지하면 상대(백돌)에게 무료 카드 1장 지급 (1회)
        ...
        if (
          ...
          totalAfterThisMove <= 8 &&
          countOpenThrees(newBoard, x, y) > 0
        ) {
          const seenIds = stateAfterCapture.usedAugmentIds[opponent];
          const ownedIdsForDraw = stateAfterCapture.ownedAugments[opponent].map((a) => a.id);
        ...
        // 양보 없음: 총 4수 이내에 흑돌이 둘 때마다 15% 확률로 상대(백돌)의 다음 턴에 보너스 착수 1회를 예약
    ...
    case "PICK_AUGMENT": {
      ...
      if (augment.id === "gambleMixed" || augment.id === "gamblePrism1") {
        ...
        if (augment.id === "gambleMixed") {
          ...
          const silvers = drawSeveralOfTier("silver", 2, [...ownedIds, "gamble"], seenIds, myColor);
          const golds = drawSeveralOfTier("gold", 1, [...ownedIds, "gamble", ...silvers.map((a) => a.id)], seenIds, myColor);
          won = [...silvers, ...golds];
        } else {
      ...
      if (augment.id === "peek") {
        ...
        const guaranteed = drawOneOfTier(effectiveRoundTier, [...ownedIdsAfter, ...stageExcludeIds], seenIds, colorForPlayer(player, state.roleSwapActive));
