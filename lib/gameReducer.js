...
export function initialGameState(isOnlineMode = false) {
  const base = {
    ...
    // 링 위에서 싸우자의 축소 시계 전용 카운터 - 실제로 보드에 새 돌이 놓일 때마다(질풍노도 보너스 착수/양수겹침
    // 2번째 착수 포함) 1씩 늘어나고 절대 줄어들지 않음. stonesPlaced(증강 선택 타이밍용)는 그 두 카드의 보너스
    // 착수를 일부러 안 세서 링이 실제보다 느리게 좁혀지는 문제가 있었고, 그렇다고 "보드 위 돌 총 개수"를 바로 쓰면
...
function triggerKarmaIfNeeded(state, victim, board) {
  ...
  for (const t of karmaTargets) board[t.y][t.x] = 0;
  return {
    removalHitCount: { ...state.removalHitCount, [victim]: hitCount },
    oneTimeUsed: oneTimeUsedWithKarma,
    forbiddenMessage: "'인과응보' 발동! 상대 돌 " + karmaTargets.length + "개가 제거됐어요!",
  };
}
...
// 시작 증강(0수, 착수 전) 뽑기도 위와 같은 이유로 이 카드들을 제외 (도박처럼 별도 선택 화면으로 이어지는 카드는
// 시작 시퀀스가 복잡해지는 걸 피하려고도 제외). 나머지 카드는 전부 정상적으로 등장 가능
// (4수마다 뜨는 회차에서는 그대로 다 등장함)
const START_DRAFT_EXCLUDE_IDS = IMMEDIATE_TARGET_EXCLUDE_IDS;
...
export function gameReducer(state, action) {
  switch (action.type) {
    case "CLICK_CELL": {
      ...
      // 둘 다 보유 중이면(둘 다 같은 "나-상대-나" 패턴을 노리는 카드라) 잡힌 돌마다 50% 확률로 어느 효과를 적용할지
      // 결정함 - 안 그러면 코드 순서상 포위 제거가 항상 먼저 그 칸을 비워버려서 오델로가 뒤집을 대상을 못 찾아
      // 완전히 죽은 카드가 됨(둘 다 프리즘 등급인데 하나가 통째로 무의미해지는 건 좋은 밸런스가 아님)
      ...
      // 양수겹침 둘째 착수 포함 - stonesPlaced와 달리 이 카운터는 그 둘도 그대로 셈). 부활로 이 수가 나중에
      ...
      if (
        ...
        if (raidImmunity.immune) {
          ...
          for (const t of raidTargets) newBoard[t.y][t.x] = 0;
          stateAfterCapture = {
            ...stateAfterCapture,
            oneTimeUsed: { ...stateAfterCapture.oneTimeUsed, [opponent]: { ...stateAfterCapture.oneTimeUsed[opponent], raid: true } },
          };
          extraQuestMessages.push("상대의 '습격'이 발동해 내 돌 " + raidTargets.length + "개가 사라졌어요!");
      ...
      if (pokerFaceBefore && !isRushBonusStone) {
        ...
        if (nextTurnsLeft > 0) {
          ...
          if (pokerFaceBefore.real) {
            ...
            // (예: 백돌 전용 '양보 없음'을 흑돌이 훔치면, 게임 로직은 항상 "지금 백돌 두는 신원"의 카드만 확인해서
    ...
    case "USE_ABILITY": {
      ...
      if (ability === "dungapsul") {
        // 자기 자신은 위장 대상에서 제외 (파기가 자기 자신을 제외하는 것과 같은 이유)
        const eligibleDisguise = state.ownedAugments[player].filter((a) => a.id !== "dungapsul");
        if (eligibleDisguise.length === 0) {
          return {
            ...state,
            forbiddenMessage: "둔갑술: 위장할 다른 증강이 없어요",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        return { ...state, pendingTarget: { player, kind: "dungapsul", need: 1, selected: [] } };
      }
    ...
    case "TARGET_CELL": {
      ...
      if (kind === "removeStone") {
        ...
        const blocked = immunity.immune || aftershockActive || preventionActive;
        ...
        // 역풍: 피해자가 역풍을 갖고 있고 이번 공격이 (면역/여진/예방 중 무엇으로든) 실제로 막혔으면,
      ...
      if (kind === "plague") {
        ...
        return {
          ...
          forbiddenToken: immunity.immune || survivorPatch || interestPatch || karmaPatch ? state.forbiddenToken + 1 : state.forbiddenToken,
          currentPlayer: immunity.immune ? player : opponent,
        };
      }
      ...
      if (kind === "ward") {
        ...
        const a = selected[0];
        if (x === a.x && y === a.y) return state; // 같은 칸 재클릭은 무시
        const dx = x - a.x;
        const dy = y - a.y;
    ...
    case "PICK_CARD_TARGET": {
      ...
      if (kind !== "discard" && kind !== "appraisal" && kind !== "lifeTransfer" && kind !== "dungapsul") return state;
      ...
      if (kind === "dungapsul") {
        if (targetCard.id === "dungapsul") {
          return {
            ...state,
            forbiddenMessage: "둔갑술 카드 자신은 위장할 수 없어요",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        // 같은 등급의 다른 카드를 위장용 가짜로 하나 뽑음 - 순수 표시용 객체라 실제 게임 로직엔 전혀 안 쓰임
        // (둔갑술 자신은 위장 이름으로 뽑히면 티가 나서 제외)
        const fakePool = AUGMENTS.filter((a) => a.tier === targetCard.tier && a.id !== targetCard.id && a.id !== "dungapsul");
        const [fake] = fakePool.length > 0 ? pickRandom(fakePool, 1) : [];
        if (!fake) {
          return {
            ...state,
            forbiddenMessage: "둔갑술: 위장에 쓸 카드가 없어서 아무 일도 일어나지 않았어요",
            forbiddenToken: state.forbiddenToken + 1,
          };
        }
        return {
          ...
          disguisedCards: {
            ...
            [player]: { ...state.disguisedCards[player], [targetCard.id]: fake },
          },
          pendingTarget: null,
          oneTimeUsed: markUsed(state, player, "dungapsul"),
          forbiddenMessage: "'둔갑술'로 '" + targetCard.name + "'을(를) 위장했어요! (상대 화면에는 다른 카드로 보여요)",
          forbiddenToken: state.forbiddenToken + 1,
        };
      }
