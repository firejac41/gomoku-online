...
// 금지 구역/영구 봉쇄는 "뽑는 즉시 칸을 지정"해야만 실제로 효과가 생기는 카드라(버튼도 없어서 나중에 따로 못 씀),
// 정상 드래프트(PICK_AUGMENT)가 아닌 다른 경로로 카드만 손에 쥐여주면 칸을 지정할 방법이 없어 영원히 죽은 카드가 됨
// - 동전 던지기/도박/잠복/역감시 보너스/파기 교체처럼 "카드만 툭 던져주는" 모든 경로에서 공통으로 제외해야 함
const IMMEDIATE_TARGET_EXCLUDE_IDS = ["banZone", "permaBlock"];

// 시작 증강(0수, 착수 전) 뽑기 - 위 두 장에 더해, 별도 선택 화면으로 이어지는(도박) 카드도 시작 시퀀스가
// 복잡해지는 걸 피하려고 제외. 나머지 카드는 전부 정상적으로 등장 가능 (4수마다 뜨는 회차에서는 그대로 다 등장함)
const START_DRAFT_EXCLUDE_IDS = [...IMMEDIATE_TARGET_EXCLUDE_IDS, "gamble"];
  ...
  switch (action.type) {
    ...
    case "PICK_AUGMENT": {
      ...
      if (augment.id === "gambleMixed" || augment.id === "gamblePrism1") {
        ...
        if (augment.id === "gambleMixed") {
          ...
          const golds = drawSeveralOfTier("gold", 1, [...ownedIds, "gamble", ...modeExclude, ...IMMEDIATE_TARGET_EXCLUDE_IDS, ...silvers.map((a) => a.id)], seenIds, myColor);
          won = [...silvers, ...golds];
        } else {
          ...
          won = Math.random() < 0.45 ? drawSeveralOfTier("prism", 1, [...ownedIds, "gamble", ...IMMEDIATE_TARGET_EXCLUDE_IDS], seenIds, myColor) : [];
        }
    ...
    case "PICK_CARD_TARGET": {
      ...
      if (kind === "discard") {
        ...
        const discardInstantPatch = activateInstantAugments(state, [newCard]);
