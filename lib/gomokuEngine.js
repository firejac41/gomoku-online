...
export const ONLINE_ONLY_IDS = ["fogZone", "cunning", "dungapsul"];
...
// 훈수/위협 확장 시야용: "열린 4"(양쪽 다 비어있는 4목 - 사실상 상대가 못 막는 필승 패턴) 개수를 세는 함수
// countOpenThrees와 마찬가지로 순수 패턴 판정이라 색을 명시해서 넘겨야 함(안 넘기면 흑돌 기준으로만 판정됨)
function countOpenFours(board, x, y, player) {
  let count = 0;
  for (const { dx, dy } of AXIS_DIRECTIONS) {
    const line = getDirectionLine(board, x, y, dx, dy, 5, player);
    if (/011110/.test(line)) count++;
  }
  return count;
}

// 훈수/위협 확장 시야: 지금 빈칸 중에 color가 두면 열린 4가 되는(=사실상 필승) 자리들을 찾기
export function findOpenFourSetupCells(board, color) {
  const cells = [];
  for (let y = 0; y < BOARD_SIZE; y++) {
    for (let x = 0; x < BOARD_SIZE; x++) {
      if (board[y][x] !== 0) continue;
      const testBoard = board.map((row) => row.slice());
      testBoard[y][x] = color;
      if (countOpenFours(testBoard, x, y, color) > 0) {
        cells.push({ x, y });
      }
    }
  }
...
// 등급 배분(총 11개→18개, 두 번째 개편): 새 가이드라인("실버는 거슬리는 정도 / 골드는 조합하면 이길 수 있는
// 실질적 효과 / 프리즘은 희귀하지만 큰 임팩트, 단 돌 갯수·자리는 못 건드림")에 맞춰 실버 4개(입김/소금 뿌리기/
// 도토리 던지기/텃세)는 유지하고 나머지 14개를 새로 추가함(자세한 배경은 PROGRESS.md 참고). 등급 배분은
// 실버 7 / 골드 7 / 프리즘 4 - 회차 등급 확률은 실버 60% / 골드 30% / 프리즘 10% 고정(gameReducer.js의
// ROUND_TIER_POOL 참고). 첫 번째 개편에서 빠진 8개(자리 바꾸기/영입/돌풍/바나나 껍질/소금 폭탄/태풍/소금비)의
// 리듀서/UI 코드는 이 배열에서 빠진 순간 도달 불가능한 죽은 코드가 되므로 안전하게 그대로 둠.
export const AUGMENTS = [
  // ── 실버 (7) ──
  {
    id: "coaching",
    name: "훈수",
    desc: "내가 다음 수로 열린 4목을 만드는 자리를 표시해줍니다.",
    tier: "silver",
  },
  {
    id: "vinegar",
    name: "초치기",
    desc: "사용: 상대의 다음 증강 선택 리롤 가능 횟수를 1회 줄입니다. (재사용 대기시간 5수)",
    tier: "silver",
  },
  {
    id: "fogZone",
    name: "안개",
    desc: "사용: 칸 1개를 지정하면 그 칸 중심 3x3 구역이 상대 화면에서만 3턴 동안 안개로 가려집니다. (재사용 대기시간 6수, 온라인 전용)",
    tier: "silver",
    onlineOnly: true,
  },
  {
  ...
  // ── 골드 (7) ──
  {
    id: "evade",
    name: "회피",
    desc: "사용: 내 돌 하나를 지정하면 다음 소유권 변경 효과 1회를 막아줍니다. (재사용 대기시간 5수)",
    tier: "gold",
  },
  {
    id: "retaliate",
    name: "역습",
    desc: "내 돌이 강탈당하면 즉시 상대 돌 하나도 함께 뺏어옵니다.",
    tier: "gold",
  },
  {
    id: "steal",
    name: "강탈",
    desc: "즉시 발동: 상대의 고립되지 않은 돌 중 3목 이상 라인에 속하지 않은 돌 하나를 지정해 데려옵니다.",
    tier: "gold",
  },
  {
    id: "threatExpand",
    name: "위협 확장 시야",
    desc: "상대의 다음 수로 열린 4목을 만드는 자리를 표시해줍니다.",
    tier: "gold",
  },
  {
    id: "cunning",
    name: "커닝",
    desc: "즉시 발동: 이후 게임이 끝날 때까지, 상대가 증강을 고르는 중일 때 그 선택지 3장이 나에게만 보입니다. (온라인 전용)",
    tier: "gold",
    onlineOnly: true,
  },
  {
    id: "tierShift",
    name: "등급 강탈",
    desc: "즉시 발동: 내 다음 증강 선택 등급을 1단계 올리고 상대의 다음 증강 선택 등급을 1단계 내립니다.",
    tier: "gold",
  },
  {
    id: "forbiddenFree",
    name: "금수 면제",
    desc: "흑돌 전용 - 보유하는 동안 나의 렌주룰 금수가 모두 해제됩니다.",
    tier: "gold",
    colorOnly: 1,
  },

  // ── 프리즘 (7) - diagBoost/othello/dungapsul/absoluteZero는 "돌 갯수·자리는 절대 건드리지 않음" 원칙을
  // 지킴. 아래 3개(무위전변/링 위에서 싸우자/폭주)는 그 원칙이 생기기 전(구 76개 체제)에 만들어진 카드를
  // 사용자 요청으로 재활성화한 것 - board를 직접 건드리는 예외로 명시적으로 남겨둠 ──
  {
    id: "diagBoost",
    name: "규칙 완화",
    desc: "대각선으로 4개만 이어도 승리합니다.",
    tier: "prism",
  },
  {
    id: "othello",
    name: "오델로",
    desc: "내 돌 사이에 상대 돌 1개가 끼면 50% 확률로 그 돌이 자동으로 내 색으로 뒤집힙니다. (2개 이상 끼면 효과 없음)",
    tier: "prism",
  },
  {
    id: "dungapsul",
    name: "둔갑술",
    desc: "즉시 발동: 이후 게임이 끝날 때까지, 내가 얻는 카드마다 상대 화면에서만 다른 카드로 위장됩니다. (온라인 전용)",
    tier: "prism",
    onlineOnly: true,
  },
  {
    id: "absoluteZero",
    name: "절대영도",
    desc: "즉시 발동: 이후 게임이 끝날 때까지, 상대가 가진 모든 액티브 능력(기존 보유분 + 앞으로 얻는 것 포함)을 사용하지 못하게 봉인합니다.",
    tier: "prism",
  },
  {
    id: "colorSwap",
    name: "무위전변",
    desc: "지금 판 위의 모든 돌 색을 서로 반전시킵니다. (흑↔백, 1회 사용, 원할 때 사용, 사용하면 턴이 넘어감)",
    tier: "prism",
  },
  {
    id: "battleRing",
    name: "링 위에서 싸우자",
    desc: "발동 즉시 판이 4수마다 사방으로 한 겹씩 좁혀 들어가 최종 8x8까지 줄어듭니다. (게임 끝까지 유지, 양쪽 모두 적용, 이미 놓인 돌은 그대로 남음)",
    tier: "prism",
  },
  {
    id: "chaos",
    name: "폭주",
    desc: "발동 즉시 게임이 끝날 때까지 양쪽 모두 조작권을 잃습니다 - 아무 칸이나 클릭해도 무작위 빈 칸에 놓이고, 증강 액티브 능력도 전부 못 씁니다.",
    tier: "prism",
  },
];

// (x,y)의 8방향 이웃에 색 무관 아무 돌이라도 있는지 (ignore 좌표는 없는 셈 치고 검사)
export function hasNeighborStone(board, x, y, ignore) {
...
// 강탈/역습용: 고립되지 않았고(주변에 다른 돌이 있고) 3목 이상 라인에 속하지 않은 이 색 돌 전부 찾기
// - 3목 이상 라인은 승부에 직결될 수 있어서 대상에서 제외하는 안전장치 (골드 등급이라 완전히 고립 돌만
// 대상으로 삼는 실버 안전 원칙보다는 느슨하지만, 이 조건으로 즉시 승리/차단 직결을 여전히 막음)
export function findStealTargets(board, color) {
  const cells = [];
  for (let y = 0; y < BOARD_SIZE; y++) {
    for (let x = 0; x < BOARD_SIZE; x++) {
      if (board[y][x] !== color) continue;
      if (!hasNeighborStone(board, x, y)) continue;
      if (getRunCounts(board, x, y, color).some((c) => c >= 3)) continue;
      cells.push({ x, y });
    }
  }
  return cells;
...
// (현재 증강 풀은 즉시발동/쿨다운/패시브 방식 액티브만 있고 1회용 능력이 없어서 비어있음)
export const ONE_TIME_ABILITY_IDS = ["colorSwap"];
