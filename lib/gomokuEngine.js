...
// 예능 증강 "액자 완성"용 도형: 3x3 테두리(가운데 뺀 8칸). 정사각형이라 회전시켜도 모양이 같음
...
export const AUGMENTS = [
  { id: "diagBoost", name: "대각선 강화", desc: "대각선 방향은 4개만 이어도 승리 (초반에만 등장)", tier: "prism" },
  { id: "straightBoost", name: "일자진", desc: "가로/세로도 4개만 이어도 승리 (초반에만 등장)", tier: "prism" },
  ...
  { id: "bind", name: "속박", desc: "사용하면 상대의 다음 턴을 통째로 건너뜁니다 (1회, 원할 때 사용)", tier: "prism" },
  ...
  { id: "watchtower", name: "감시탑", desc: "빈 칸 1곳을 지정 (양쪽에 표시됨). 상대가 4턴 안에 그 칸에 두면 그 수는 무효화됨 (1회, 원할 때 사용)", tier: "gold" },
  ...
  { id: "doubleMove", name: "양수겹침", desc: "한 턴에 돌을 2개 놓기 (게임 중 1회, 원할 때 사용)", tier: "silver" },
  ...
  { id: "frameWin", name: "액자 완성", desc: "3x3 테두리(가운데 뺀 8칸)를 전부 내 돌로 채우면 즉시 승리", tier: "silver", shape: FRAME_SHAPE, shapeGrid: 3 },
];
...
// 도형 증강(액자 완성 등)용: shapeCells를 board 어딘가에 옮겨서 전부 player 돌로 채워졌는지 검사 (평행이동만 검사, 회전은 안 함)
export function checkShapeWin(board, player, shapeCells) {
  const maxDx = Math.max(...shapeCells.map((c) => c.x));
  const maxDy = Math.max(...shapeCells.map((c) => c.y));
  for (let ay = 0; ay <= BOARD_SIZE - 1 - maxDy; ay++) {
    for (let ax = 0; ax <= BOARD_SIZE - 1 - maxDx; ax++) {
      if (shapeCells.every(({ x, y }) => board[ay + y][ax + x] === player)) return true;
    }
  }
  return false;
}
