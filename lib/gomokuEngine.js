...
// 실제 플레이어 반응은 그런 확정적 파워보다 슴슴하고 단순한 판을 원한다는 쪽이었음. 그래서 기존 카드를
// 전부 뽑기 풀에서 비활성화(이 배열에서 삭제)하고, 새 철학의 카드로 교체함: "발동해도 승부를 뒤집거나
// 확정 승리를 만들 수는 없지만, 상대 기분은 살짝 나쁘게 만드는" 순수 눈치/연출용 카드. 그래서 이번 3개는
// 전부 board/승리 조건/턴 순서 등 실제 게임 상태를 절대 건드리지 않는 "장난성" 액티브로만 구성함
// (재사용 대기시간 있음, 사용해도 턴은 안 넘어감 - 대비태세와 같은 패턴). 기존 카드들의 리듀서/UI 코드는
// 이 배열에서 빠진 순간 전부 도달 불가능한 죽은 코드가 되므로 안전하게 그대로 둠(별도 삭제 없이 이 배열
// 하나만 교체하면 게임에서 완전히 사라짐) - 나중에 다시 늘려나갈 예정이라 "일단 조금만"으로 3개만 추가함.
export const AUGMENTS = [
  {
    id: "jeer",
    name: "약올리기",
    desc: "쿨다운 4수 · 사용해도 턴은 안 넘어감\n무작위 도발 메시지를 띄움 (게임 진행에는 전혀 영향 없음)",
    tier: "silver",
  },
  {
    id: "startle",
    name: "놀래키기",
    desc: "쿨다운 4수 · 사용해도 턴은 안 넘어감\n판 전체가 잠깐 흔들리는 연출 (게임 진행에는 전혀 영향 없음)",
    tier: "silver",
  },
  {
    id: "firecracker",
    name: "폭죽 세례",
    desc: "쿨다운 4수 · 사용해도 턴은 안 넘어감\n보드 위에 폭죽 연출이 터짐 (게임 진행에는 전혀 영향 없음)",
    tier: "silver",
  },
];

// 무작위 도발 문구 - "약올리기" 사용 시 이 중 하나가 뽑혀 forbiddenMessage로 표시됨
export const JEER_LINES = [
  "메롱~ 그 정도 수로 되겠어?",
  "어이쿠, 그건 좀 아쉬운데?",
  "훗, 긴장되나 보네?",
  "잘 두고 있어? 나는 여유로운데~",
  "그 수 진심이야?",
  "손이 떨리는 소리가 여기까지 들리는데?",
];
