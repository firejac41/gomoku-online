...
const ENHANCED_DESCRIPTIONS = {
  ...
  balance: "내 돌이 상대보다 1개(강화) 이상 적으면, 그동안 렌주룰 금수가 면제됨",
...
export function getRingBounds(ringStartMove, totalStonesPlaced) {
  const level = getRingLevel(ringStartMove, totalStonesPlaced);
  if (level === 0) return null;
  const left = Math.ceil(level / 2);
  const right = Math.floor(level / 2);
  const top = Math.ceil(level / 2);
  const bottom = Math.floor(level / 2);
  return { minX: left, maxX: BOARD_SIZE - 1 - right, minY: top, maxY: BOARD_SIZE - 1 - bottom };
}
...
export const AUGMENTS = [
  ...
  { id: "battleRing", name: "링 위에서 싸우자", desc: "발동 즉시 판이 4수마다 사방으로 한 겹씩 좁혀 들어가 최종 8x8까지 줄어듦 (게임 끝까지 유지, 양쪽 모두 적용, 이미 놓인 돌은 그대로 남음)", tier: "prism" },
