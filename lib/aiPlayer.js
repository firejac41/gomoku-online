...
// 봉쇄 계열(금지구역/영구봉쇄/감시탑) 대상 칸은 board를 안 건드리고 blockedCells/permaBlockedCells/
// watchtowerCells에만 추가돼서 evalStateForPlayer(=fullBoardScore 차이)가 후보마다 사실상 동일하게 나옴 -
// 그러면 기존엔 Math.random() 지터로 거의 무작위 칸을 골랐음. 상대 돌 군집에 가까운 칸일수록 상대가
// 실제로 뭔가 만들고 있는 곳을 막는 셈이라 가치가 높다는 휴리스틱으로 타이브레이크를 대체함.
const BLOCKING_TARGET_KINDS = new Set(["banZone", "permaBlock", "watchtower"]);
