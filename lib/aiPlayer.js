...
import { gameReducer, getActiveAugmentIds, getLegalCells } from "./gameReducer";
...
function scoreAugmentForPick(state, player, augment) {
  ...
  // 도미노는 포위 제거/오델로로 상대 돌을 없앨 때만 보너스 착수가 생김 - 조합 파츠가 없으면 사실상 죽은 카드라
  // 이미 그 둘 중 하나를 갖고 있으면 강하게 가산, 없으면 프리즘 값에서 골드 쪽으로 끌어내려 과대평가를 막음
  // (완전히 안 뽑히게 죽이지는 않음 - 나중에 포위 제거/오델로를 노리고 투기적으로 집는 것도 합리적이므로)
  if (augment.id === "domino") {
    const owned = state.ownedAugments[player] || [];
    const hasComboPiece = owned.some((c) => c.id === "capture" || c.id === "othello");
    score += hasComboPiece ? 8 : -10;
  }
  return score;
}
...
const ABILITY_PRIORITY = {
  ...
  // selfUndo는 "내 마지막 수"를 되돌릴 뿐 상대 위협과는 무관함(undo/removeStone/plague/collapse처럼
  // 상대 돌/수를 직접 지우는 카드가 아님) - 예전엔 다른 방어형 카드들과 같은 패턴으로 ctx.oppThreatens일 때
  // 80점(사용 임계값 20을 훌쩍 넘김)을 줬는데, 실제로는 상대 위협을 전혀 막지 못하면서 되돌린 자리에 그대로
  // 다시 두는 경우가 대부분이라(내 돌 하나만 순수하게 손해보고 나머지 수순은 동일) 매 판 자기 자신을
  // 갉아먹는 오작동이었음. 이 휴리스틱 AI는 "그 자리가 사실 나쁜 수였다"를 감지할 방법이 없어서 항상 낮게 둠
  selfUndo: () => -100,
...
// 봉쇄 계열(금지구역/영구봉쇄/감시탑/결계) 대상 칸은 board를 안 건드리고 blockedCells/permaBlockedCells/
// watchtowerCells/deadCells에만 추가돼서 evalStateForPlayer(=fullBoardScore 차이)가 후보마다 사실상 동일하게 나옴 -
// 그러면 기존엔 Math.random() 지터로 거의 무작위 칸을 골랐음. 상대 돌 군집에 가까운 칸일수록 상대가
// 실제로 뭔가 만들고 있는 곳을 막는 셈이라 가치가 높다는 휴리스틱으로 타이브레이크를 대체함.
// 결계(ward)도 완전히 같은 문제(두 지정 칸이 board를 안 건드림)를 겪는데 처음 추가될 때 이 목록에서 빠져 있었음 -
// 실측해보니 상대 위협과 무관한 빈 구석을 골라버리는 것을 확인해서 여기에 포함시킴(정확한 축 정렬까지는
// 못 잡아도, 최소한 상대 돌 군집 근처를 고르게는 만들어줌)
const BLOCKING_TARGET_KINDS = new Set(["banZone", "permaBlock", "watchtower", "ward"]);

function nearestOpponentStoneDist(board, x, y, oppColor) {
  let best = Infinity;
  for (let yy = 0; yy < BOARD_SIZE; yy++) {
    for (let xx = 0; xx < BOARD_SIZE; xx++) {
      if (board[yy][xx] !== oppColor) continue;
      const d = Math.max(Math.abs(xx - x), Math.abs(yy - y)); // 체비쇼프 거리
      if (d < best) best = d;
    }
  }
  return best;
...
function blockingTargetBonus(state, aiPlayer, x, y) {
  const oppColor = colorForPlayer(otherPlayer(aiPlayer), state.roleSwapActive);
  const d = nearestOpponentStoneDist(state.board, x, y, oppColor);
  if (d === Infinity) {
    // 상대 돌이 아직 하나도 없으면 중앙 근처를 약하게 선호(그래도 무작위보단 나음)
    return -(Math.abs(x - CENTER) + Math.abs(y - CENTER)) * 0.5;
  }
  return -d * 4; // 가까울수록(거리가 작을수록) 높은 점수 - 지터(±2)보다 크게 잡아 실질 타이브레이커가 됨
}

function resolvePendingTarget(state, aiPlayer, depth = 0, anchor = null) {
  const candidates = candidateCellsForTarget(anchor);
  const isBlockingTarget = state.pendingTarget && BLOCKING_TARGET_KINDS.has(state.pendingTarget.kind);
  let best = null;
  for (const c of candidates) {
    ...
    // 봉쇄 계열이면 이 칸 자체의 상대 근접도 보너스를 더함(banZone처럼 여러 칸이면 각 칸이 자기 몫을 기여)
    if (isBlockingTarget) score += blockingTargetBonus(state, aiPlayer, c.x, c.y);
    score += Math.random() * 2;
